...
export default function ContactActionBar() {
  ...
  return (
    ...
    {isFormOpen && (
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/55 px-4 py-6 font-[var(--font-geist)] backdrop-blur-[3px]" role="dialog" aria-modal="true" aria-labelledby="contact-form-title" onMouseDown={(event) => { if (event.target === event.currentTarget) setIsFormOpen(false); }}>
        <div className="relative w-full max-w-[480px] rounded-[26px] bg-white p-6 shadow-[0_24px_70px_rgba(0,0,0,.28)] sm:p-8">
          <button type="button" aria-label="Close form" onClick={() => setIsFormOpen(false)} className="absolute right-4 top-4 grid h-9 w-9 place-items-center rounded-full bg-[#f4f6f5] text-[#33433c] transition hover:bg-[#e9eeeb]"><X className="h-5 w-5" /></button>
          <div className="pr-10">
          ...
          <form className="mt-6 grid gap-4" onSubmit={(event) => { event.preventDefault(); setIsFormOpen(false); }}>
            ...
            <button type="submit" className="mt-2 inline-flex h-12 items-center justify-center gap-2 rounded-full bg-[linear-gradient(135deg,#e13e20,#b92f18)] px-6 text-sm font-semibold text-white shadow-[0_7px_18px_rgba(225,62,32,.22)] transition hover:brightness-105">Submit Request <Send className="h-4 w-4" /></button>
          </form>
        </div>
      </div>
    )}

...
const stories = [
  { image: "https://ik.imagekit.io/wwdlbhsjw/public/main1.jpg", views: "145K Views" },
  { image: "https://ik.imagekit.io/wwdlbhsjw/public/main2.jpg", views: "99K Views" },
  { image: "https://ik.imagekit.io/wwdlbhsjw/public/main3.jpg", views: "78K Views" },
  { image: "https://ik.imagekit.io/wwdlbhsjw/public/main4.jpg", views: "112K Views" },
  { image: "https://ik.imagekit.io/wwdlbhsjw/public/main5.jpg", views: "89K Views" },
];
      ...
      <div className="mx-auto max-w-[1500px]">
        ...
        <div className="px-3 pt-5 sm:px-7 lg:pt-12">
          ...
          <div className="relative">
            <button type="button" aria-label="Previous stories" onClick={() => move(-1)} className="absolute left-1 top-1/2 z-10 grid h-9 w-9 -translate-y-1/2 place-items-center rounded-full border border-[#e8ece9] bg-white/95 text-[#5b8977] shadow-md backdrop-blur-sm transition hover:scale-105 sm:left-0 sm:-translate-x-1/2 lg:h-11 lg:w-11"><ArrowRight className="h-4 w-4 rotate-180 lg:h-5 lg:w-5" /></button>

            <div ref={carouselRef} className="grid snap-x snap-mandatory grid-flow-col auto-cols-[100%] gap-4 overflow-x-auto px-0 pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:auto-cols-[calc(50%-10px)] sm:gap-5 md:auto-cols-[30%] lg:auto-cols-[calc((100%-80px)/5)]">
              {stories.map(({ image, views }, index) => (
                <article key={image} className="group relative aspect-[.8] snap-start overflow-hidden rounded-[15px] bg-[#ddd] shadow-sm">
                  <img src={image} alt={`Ayush Ortho patient story ${index + 1}`} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/75 via-black/5 to-transparent" />
                  <button type="button" aria-label={`Play patient story ${index + 1}`} className="absolute left-1/2 top-1/2 grid h-9 w-9 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full border-2 border-white/90 bg-black/15 text-white backdrop-blur-[1px] transition group-hover:scale-110 sm:h-11 sm:w-11"><Play className="ml-0.5 h-4 w-4 sm:h-5 sm:w-5" fill="white" /></button>
                  <div className="absolute bottom-2 left-2 flex items-center gap-1.5 text-[9px] font-semibold text-white sm:bottom-3 sm:left-3 sm:text-[10px]"><Play className="h-3 w-3 sm:h-4 sm:w-4" fill="white" />{views}</div>
                </article>
              ))}
            </div>
